...
abstract type AbstractTogetherProvider end
Base.@kwdef struct TogetherProvider <: AbstractTogetherProvider
    api_key::String = ""
    base_url::String = "https://api.together.xyz"
    api_version::String = ""
end
...
Default provider for Together API requests.
"""
const DEFAULT_PROVIDER = let 
    api_key = get(ENV, "TOGETHER_API_KEY", nothing)
    if api_key === nothing
        TogetherProvider()
    else
        TogetherProvider(api_key=api_key)
    end
end

"""
    auth_header(provider::AbstractTogetherProvider, api_key::AbstractString)

Return the authorization header for the given provider and API key.
"""
auth_header(provider::AbstractTogetherProvider) = auth_header(provider, provider.api_key)
function auth_header(::TogetherProvider, api_key::AbstractString)
...
"""
    build_url(provider::AbstractTogetherProvider, api::AbstractString)

Return the URL for the given provider and API.
"""
build_url(provider::AbstractTogetherProvider) = build_url(provider, provider.api)
function build_url(provider::TogetherProvider, api::String)
